...
from c3.libraries.envelopes import gauss_white_noise, gaussian_sigma
...
if __name__ == '__main__':
    ...
    t_final = gate.t_end
    amp = Quantity(value=0.0, min_val=0.0, max_val=1., unit="V")
    params = {'amp': amp,
    ...
    env = Envelope(name='white_noise', normalize_pulse=False, params=params, shape=gauss_white_noise)
    ...
    for amp_val in amps:
        ...
        fids.append(avg_fid)
        print(f'Fid for white noise of amplitude {amp.get_value():.2e}: {avg_fid}')
    ...
    plt.grid()
    plt.title('Infidelity for different white noise for doubly resonant CX')
